...
class ARM_Pagseguro{
    ...
    function __construct() {
        ...
        register_activation_hook(__FILE__, array('ARM_Pagseguro', 'arm_pagseguro_check_network_activation'));
	...
	function upgrade_data_pagseguro() {
            ...
            if (version_compare($armnew_pagseguro_version, '1.9', '<')) {
     ...
     */
    public static function arm_pagseguro_check_network_activation($network_wide) {
        if (!$network_wide)
            return;
        ...
        if ($payment_gateway == 'pagseguro' && isset($all_payment_gateways['pagseguro']) && !empty($all_payment_gateways['pagseguro'])) {
            ...
            if (!empty($entry_data) && $current_payment_gateway == $payment_gateway) {
                ...
                if ($recurring_payment_mode == 'manual_subscription') {
                    ...
                    $response = $createpaymentrequest->main($data_array, $recurring_payment_mode);
                    ...
                    if ($response['status'] == 'success') {
                        ...
                        $ARMember->arm_write_response("pagseguro subscription log response 1=> " . maybe_serialize($response));
                ...
                else 
                {
                    ...
                    else
                    {
                        ...
                        if ($response['status'] == 'success' && isset($response['redirect_url']['checkoutUrl'])) {
                            ...
                            $ARMember->arm_write_response("pagseguro subscription log response 2=> " . maybe_serialize($response));
...
//                $arm_redirecturl = get_permalink($redirect_page_id);
                ...
                if ((($discount_amt <= 0 || $discount_amt == '0.00') && $recurring_payment_mode == 'manual_subscription' && $plan->is_recurring()) || (!$plan->is_recurring() && ($discount_amt <= 0 || $discount_amt == '0.00'))) {
                    ...
                    $pagseguro_response['arm_first_name']=$arm_first_name;
                    $pagseguro_response['arm_last_name']=$arm_last_name;
                ...
                if($is_recurring && $recurring_payment_mode == 'auto_debit_subscription' && !empty($recurring_data))
                {
                    ...
                    else
                    {
                        ...
                        if ($response['status'] == 'success' && isset($response['redirect_url']['checkoutUrl'])) {
                            ...
                            $ARMember->arm_write_response("pagseguro subscription log response 3=> " . maybe_serialize($response));
                ...
                else 
                {
                    ...
                    if ($response['status'] == 'success') {
                        ...
                        $ARMember->arm_write_response("pagseguro subscription log response 4=> " . maybe_serialize($response));
    ...
    function arm2_add_user_and_transaction($entry_id = 0, $tran_id = '', $arm_display_log = 1) {
		...
		$user_detail_last_name = get_user_meta( $user_id, 'last_name', true);
                ...
                $pagseguroLog['arm_first_name']=$user_detail_first_name;
                $pagseguroLog['arm_last_name']=$user_detail_last_name;
    ...
    function arm_store_pagseguro_log($pagseguro_response = '', $user_id = 0, $plan_id = 0, $extraVars = array(), $arm_display_log = '1') {
        ...
        if (!empty($pagseguro_response) && empty($transaction)) {
            $payment_data = array(
                'arm_user_id' => $user_id,
                'arm_first_name'=>$pagseguro_response['arm_first_name'],
                'arm_last_name'=>$pagseguro_response['arm_last_name'],
                        ...
                        if ($payment_type == 'pagseguro') {
                            ...
                            if($user_selected_payment_mode == 'auto_debit_subscription') {
                                ...
                                try{
                                    ...
                                    $ARMember->arm_write_response("reputelog ========== Cancel Payment ========> ".maybe_serialize($e));
                                }
...
class CreatePaymentRequest {
    ...
    public static function main($data_array, $payment_mode = '') {
...
class CreatePreApproval {
    public static function main($data_array) {
        ...
        $data_array['item_amount'] = str_replace(',','',$data_array['item_amount']);
        ...
        $data_array['max_total_amount'] = str_replace(',','',$data_array['max_total_amount']);
